...
## HMS previewer 合成攻关（2026-06-10）

承上：复杂 app 的 `@kit.*`/`@hms.*` 渲染。逐层啃通了 DevEco 的 HMS previewer 合成：

- **HMS previewer 是叠加包**（无 rich 二进制）：rich `Previewer` 来自 openharmony；HMS 提供
  `module/`(原生)、`systemHsp/`(系统组件 .hsp)、`apiMock/`。
- **合成配方（已验证 `@hms:*` 模块可解析）**，见 `scripts/compose-hms-previewer.sh`：
  1. 走 **debug 路径**（`-d` → ability_simulator 正确加载 Stage ETS；非 debug 走 JS 前端会
     `Compile js bundle failed`/manifest.json 报错）。
  2. **合并 module**：把 openharmony + HMS 的 `module/*` 并入一个目录，以它为 Previewer 的 cwd
     （二进制 @rpath 与 cwd 无关，可异地）。
  3. **`-hsp <hms/previewer 目录>`**：系统 HSP 解析（代码自动追加 `/systemHsp`；勿传到 systemHsp 子层）。
  4. `-ljPath`（pkgContextInfo）+ 归一化 ohmurl abp。
- 攻关结果：`@hms:*` 系统模块**成功解析**（合成生效，不再 Cannot find module）。
- **最终拦路 = 预览 systemHsp 缺组件（非版本、非工具问题）**，证据链完整：
  - 基线 SDK **版本匹配**：app targetSdkVersion `6.1.0(23)` = 已装 DevEco SDK `HarmonyOS 6.1.0/API23`。
  - 编译期 API 一致：`@kit.UIDesignKit.d.ts` 声明 `HdsNavigation` 来自 `@hms.hds.hdsBaseComponent`，
    `@hms.hds.hdsBaseComponent.d.ets` 也确有该导出；app 的 abc 正确 import 自 `@hms:hds.hdsBaseComponent`。
  - 但**运行期预览 systemHsp 中根本没有 `hdsBaseComponent.hsp`**（全 SDK 搜不到）——只打包了
    `HdsComponent.hsp`/`HdsAesthetic*.hsp`（packageName=HdsComponent，非 hdsBaseComponent）。
    `HdsNavigation` 的 abc 实际在 `HdsComponent.hsp` 内，但 import 按包名 `hdsBaseComponent` 找不到它。
  - ⇒ `SyntaxError: '@hms:hds.hdsBaseComponent' does not provide export 'HdsNavigation'`。
  - 即 DevEco 独立 SDK 的**预览 systemHsp 漏打了 `hdsBaseComponent` 这个预览 HSP**；DevEco GUI 预览
    应是另行下载/生成该 HSP（HMS 服务 HSP），而本机无 CLI sdkmgr（仅 GUI SDK Manager + 华为账号可补）。
  - 结论：换 SDK *版本* 无用（版本已匹配）；缺的是某个预览 HSP 组件，受 DevEco GUI 预览管线牵制。
- **铁证（DevEco 自身复现）**：用 DevEco GUI 生成的 `.preview` 构建（含 `FakeUIAbility` 包壳 +
  preview abc + loader）跑我的工具链，得到**完全相同的** `does not provide HdsNavigation` 报错 ——
  即 DevEco 官方预览这个 app 也卡在同一处（用户实测 GUI 亦"无法预览"）。⇒ 工具链已被验证能驱动
  DevEco 的真实预览产物；拦路 100% 是 app+SDK 的预览 systemHsp 缺组件，与工具无关。
- DevEco 预览机制澄清（旁证）：ArkUI 预览是**按组件**（`@Entry`/`@Preview`），DevEco 生成
  `.preview/fakeuiability/FakeUIAbility.ets`（`loadContent('page/SplashPage')`）+ 专用 preview abc
  （`.preview/.../assets/.../modules.abc`，非 assembleHap 的 loader_out）。我的工具链对接此产物即可。
- 工具侧产出（与具体 app 无关、可复用）：`previewer-host` 新增 `--hsp`；
  `scripts/compose-hms-previewer.sh` 复用合成配方。⇒ 预览 systemHsp 完整的 HMS 应用，用此配方即可渲染。

## 后续（Backlog）

- HMS 原生 module 合并自动化（host 内置 compose，免手动脚本）+ apiMock(`-ilt`/jsMockHmos) 接入。
- break-on-start 批量跨过（复杂 app 模块多，逐个 continue 慢；加 `stopOnEntry=false`/快速放行）。
