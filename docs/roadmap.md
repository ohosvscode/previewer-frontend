...
- 攻关结果：`@hms:hds.hdsBaseComponent` 等系统模块**成功解析**（不再 Cannot find module）。
- **最终拦路 = SDK 版本不匹配**（非工具/合成问题）：`sample_in_harmonyos` 的 `@hms:hds.*` 需要导出
  `HdsNavigation` 的 UIDesignKit 版本，而 DevEco 6.1 的 systemHsp 中**不含该导出**（确认无任何 .hsp
  导出 HdsNavigation）→ `SyntaxError: does not provide export 'HdsNavigation'`。属 SDK 供给问题。
- 工具侧产出：`previewer-host` 新增 `--hsp`；`scripts/compose-hms-previewer.sh` 复用配方。
  ⇒ 系统组件版本匹配的 HMS 应用，用此配方即可渲染。
