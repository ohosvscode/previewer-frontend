...
**M0、M1 已完成** ✅（2026-06-06）。可运行的端到端只读预览已跑通：

```bash
cd host && cargo run --bin previewer-host   # 启动 Host（spawn 开源 Simulator + WsGateway）
# 浏览器打开 http://127.0.0.1:9000 即见 liteWearable 实时画面
```

- **M0**：协议探针端到端验证（`host/src/bin/m0_probe.rs`）——两条通道 + sid + 大端帧 + JPEG。
- **M1**：Host（Rust core + WsGateway）+ vanilla UI（Transport 抽象 + ScreenCanvas + 圆屏外框）。

下一步 **M2 可交互**（命令通道下行 + 指针/路由）。实施路线见 [`docs/roadmap.md`](docs/roadmap.md)。
